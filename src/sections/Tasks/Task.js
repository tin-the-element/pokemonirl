...
function Task() {
  ...
  useEffect(() => {
    const fetchProblem = async () => {
      try {
        ...
        const singleTask = await API.graphql({
          query: queries.getSingleTask,
          variables: { id: id },
        });
    ...
    currentHP,
    turnsLeft,
    singleTaskData.turns_permitted,
    singleTaskData.total_hp,
    usersPokemons,
    selectedPokemon,
    history,
    singleTaskData.exp_given,
    singleTaskData.lose_quote,
    singleTaskData.name,
    singleTaskData.win_quote,
  ]);
  ...
  function try_attack(move) {
    ...
    if (move.type.toLowerCase() === singleTaskData.answer[0].toLowerCase()) {
      ...
      setPostMoveMessage(
        "Your attack was effective and it did " + move.power + " damage!"
      );
    } else {
        ...
        <div class="battle">
          <div class="problem">
            ...
            <div
              ...
              style={{
                backgroundImage:
                  "url(/assets/single_tasks/" +
                  singleTaskData["images"] +
                  ".jpg",
              }}
            >
              <img
                id="battle_image"
                class="battle_image"
                alt={singleTaskData.name}
                src={
                  "/assets/single_tasks/" + singleTaskData["images"] + ".jpg"
                }
              ></img>
            ...
            <progress
              id="hp_bar"
              value={currentHP}
              max={singleTaskData.total_hp}
            ></progress>
            <div>
              {currentHP}/{singleTaskData.total_hp}
            </div>
