...
function UsersPokemon() {
  ...
  function mainSelected(pokemon) {
    if (chosenMainPokemon !== "") {
      document.getElementById(
        "Main" + chosenMainPokemon
      ).style.backgroundColor = "initial";
      document.getElementById("Main" + chosenMainPokemon).style.color =
        "inherit";
    }
  ...
  function allSelected(pokemon) {
    if (chosenAllPokemon !== "") {
      document.getElementById("All" + chosenAllPokemon).style.backgroundColor =
        "initial";
  ...
  async function swapPokemon() {
    ...
    if (mainPokemonIds.includes(chosenAllPokemon)) {
      setErrorMessage(
        "This pokemon is already in your party, please try again"
      );
      return;
    }
    ...
    const newAccountData = {
      ...
      users_pokemon: pokemonIds,
      main_pokemon: mainPokemonIds,
  ...
  async function addPokemon() {
    ...
    if (mainPokemonIds.length > 18) {
      setErrorMessage(
        "There are too many pokemon in your party, please choose one to swap"
      );
      return;
    }
    mainPokemonIds.push(chosenAllPokemon);
  ...
  return (
    <div className="center-div">
      {mainPokemon.length === 0 || pokemons.length === 0 ? (
        <MoonLoader color={"white"} loading={"true"} size={150} />
      ) : (
        <div className="center-div">
          <div className="pokemon_title_div">
            {" "}
            <h1>Your Pokemon</h1>
          </div>
          <h1>Main Pokemon</h1>
          <div className="pokemon-list-container">
            {mainPokemon.map((user_pokemon, index) => (
              <div
                id={"Main" + user_pokemon.id}
                onClick={() => mainSelected(user_pokemon.id)}
                className="pokemon center-div"
                key={user_pokemon.id ? user_pokemon.id : index}
              >
                <h1 style={{ marginBottom: "0px" }}>{user_pokemon.nickname}</h1>
                <h3>{toTitleCase(user_pokemon.pokemon.replace("-", " "))}</h3>
                <img
                  alt={user_pokemon.pokemon.replace("-", " ")}
                  src={user_pokemon.image}
                ></img>
          ...
          <div className="pokemon-list-container">
            {pokemons.map((user_pokemon, index) => (
              <div
                id={"All" + user_pokemon.id}
                onClick={() => allSelected(user_pokemon.id)}
                className="pokemon center-div"
                key={user_pokemon.id ? user_pokemon.id : index}
              >
                <h1 style={{ marginBottom: "0px" }}>{user_pokemon.nickname}</h1>
