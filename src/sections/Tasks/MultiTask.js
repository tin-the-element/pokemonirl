...
function MultiTask() {
  ...
  useEffect(() => {
    const fetchProblem = async () => {
      try {
        ...
        const multiTask = await API.graphql({
          query: queries.getMultipleTask,
          variables: { id: id },
        });
    ...
    currentStep,
    turnsLeft,
    multiTaskData.turns_permitted,
    multiTaskData.total_hp,
    usersPokemons,
    selectedPokemon,
    history,
    multiTaskData.exp_given,
    multiTaskData.lose_quote,
    multiTaskData.name,
    multiTaskData.win_quote,
  ]);
  ...
  function try_attack(move) {
    ...
    if (
      ...
      multiTaskData.answer[currentStep].toLowerCase()
    ) {
        ...
        <div class="battle">
          <div class="problem">
            ...
            <div
              ...
              style={{
                backgroundImage:
                  "url(/assets/multi_tasks/" +
                  multiTaskData["images"] +
                  currentStep +
                  ".jpg",
              }}
            >
            ...
            <div class="step_grid">
              {multiTaskData.answer.map((step, key) => {
                if (key === currentStep) {
                  return (
                    <div class="step_box current_step_box" key={key}></div>
                  );
                } else if (key < currentStep) {
                  return (
                    <div
                      className={"step_box " + step.toLowerCase() + "_step_box"}
                      key={key}
                    ></div>
                  );
                } else {
                  return <div className={"step_box"} key={key}></div>;
                }
              })}
            </div>
